...
def pipeline(arch: str):
    ...
    for (kconfig, stage4_name, profile, stage3_name) in stage4_list:
        ...
        (valid, archive_name) = save_tarball(arch, profile, stage_define = stage3_name, upstream = False, patches = patches, patches_have_been_compiled = True, kconfig = kconf, strip_deps = True, friendly_name = stage4_name, custom_stage = stage3_name)
        if not valid:
